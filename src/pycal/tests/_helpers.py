...
def create_outdir(mpicomm, subdir=None):
    ...
    """
    pwd = os.path.abspath(".")
    testdir = os.path.join(pwd, "cal_test_output")
    retdir = testdir
...
def create_comm(mpicomm):
    ...
    Returns:
        cal.Comm: the 2-level toast communicator.

    """
    calcomm = None
    if mpicomm is None:
        calcomm=Comm(world=mpicomm)
    else:
        worldsize = mpicomm.size
        groupsize = 1
        if worldsize >= 2:
            groupsize = worldsize // 2
        calcomm = Comm(world=mpicomm, groupsize=groupsize)
    return calcomm
    
def create_distdata(mpicomm, obs_per_group=1):
    """Create a cal communicator and distributed data object.
    ...
    Returns:
        cal.Data: the distributed data with named observations (but no TOD).

    """
    calcomm = create_comm(mpicomm)
    data = Data(calcomm)
    
    for obs in range(obs_per_group):
        ob = {}
        ob["name"] = "test-{}-{}".format(calcomm.group, obs)
        ob["id"] = obs_per_group * calcomm.group + obs
        data.obs.append(ob)
    return data
...
def boresight_focalplane(ndet, samplerate=1.0, epsilon=0.0, net=1.0, fmin=0.0, alpha=1.0, fknee=0.05):
