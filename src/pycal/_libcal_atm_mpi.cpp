...
void init_mpi_atm(py::module & m) {
#ifdef HAVE_CHOLMOD
    py::class_ <cal::mpi_atm_sim, cal::mpi_atm_sim::puniq> (
        m, "AtmSimMPI",
        R"(
        ...
        py::init(
            [](double azmin, double azmax, double elmin, double elmax,
               ...
               double rmin, double rmax) {
                MPI_Comm comm = cal_mpi_extract_comm(pycomm);
                return new cal::mpi_atm_sim(
    ...
    .def("simulate", &cal::mpi_atm_sim::simulate, py::arg(
             "use_cache"), R"(
    ...
    .def("observe", [](cal::mpi_atm_sim & self, py::buffer times,
...
PYBIND11_MODULE(_libcal_mpi, m) {
    m.doc() = R"(
    Interface to C++ CAL MPI library.
