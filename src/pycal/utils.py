...
def set_numba_threading():
    ...
    threads used by cal.  Since cal does not use numba, it means that there will
    ...
    # Get the number of threads used by cal at runtime.
    env = Environment.get()
    log = Logger.get()
    calthreads = env.max_threads()
    ...
    try:
        ...
        config.NUMBA_DEFAULT_NUM_THREADS = calthreads
        config.NUMBA_NUM_THREADS = calthreads
        os.environ["NUMBA_THREADING_LAYER"] = threading
        os.environ["NUMBA_DEFAULT_NUM_THREADS"] = "{:d}".format(calthreads)
        os.environ["NUMBA_NUM_THREADS"] = "{:d}".format(calthreads)
