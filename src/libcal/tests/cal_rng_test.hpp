...
const int64_t CALrngTest::size = 11;
const uint64_t CALrngTest::counter[] = {1357111317, 888118218888};
const uint64_t CALrngTest::key[] = {3405692589, 3131965165};
const uint64_t CALrngTest::counter00[] = {0, 0};
const uint64_t CALrngTest::key00[] = {0, 0};
...
void CALrngTest::SetUp() {
    return;
}

TEST_F(CALrngTest, reprod) {
    double result1[size];
    double result2[size];

    cal::rng_dist_normal(size, key[0], key[1], counter[0], counter[1],
                           result1);
    cal::rng_dist_normal(size, key[0], key[1], counter[0], counter[1] + 5,
                           result2);
...
TEST_F(CALrngTest, reprod_multi) {
    ...
    cal::AlignedVector <double> data(nfull);
    ...
    cal::rng_multi_dist_normal(nstream, ndata, mkey1, mkey2, mcounter1,
                                 mcounter2, pdata);

    // Compare this to the result of generating the whole stream serially.

    double compare[nfull];
    cal::rng_dist_normal(nfull, key[0], key[1], counter[0], counter[1],
                           compare);
...
TEST_F(CALrngTest, uniform11) {
    double result[size];

    cal::rng_dist_uniform_11(size, key[0], key[1], counter[0], counter[1],
                               result);
    ...
    cal::rng_dist_uniform_11(size, key00[0], key00[1], counter00[0],
                               counter00[1], result);
...
TEST_F(CALrngTest, uniform11_multi) {
    ...
    cal::rng_multi_dist_uniform_11(nstream, ndata, mkey1, mkey2, mcounter1,
                                     mcounter2, pdata);

    // Compare this to the result of generating the whole stream serially.

    double compare[nfull];
    cal::rng_dist_uniform_11(nfull, key[0], key[1], counter[0], counter[1],
                               compare);
...
TEST_F(CALrngTest, uniform01) {
    double result[size];

    cal::rng_dist_uniform_01(size, key[0], key[1], counter[0], counter[1],
                               result);
    ...
    cal::rng_dist_uniform_01(size, key00[0], key00[1], counter00[0],
                               counter00[1], result);
...
TEST_F(CALrngTest, uniform01_multi) {
    ...
    cal::rng_multi_dist_uniform_01(nstream, ndata, mkey1, mkey2, mcounter1,
                                     mcounter2, pdata);

    // Compare this to the result of generating the whole stream serially.

    double compare[nfull];
    cal::rng_dist_uniform_01(nfull, key[0], key[1], counter[0], counter[1],
                               compare);
...
TEST_F(CALrngTest, uint64) {
    uint64_t result[size];

    cal::rng_dist_uint64(size, key[0], key[1], counter[0], counter[1],
                           result);
    ...
    cal::rng_dist_uint64(size, key00[0], key00[1], counter00[0],
                           counter00[1], result);
...
TEST_F(CALrngTest, uint64_multi) {
    ...
    // The data buffer we will process in segments
    size_t nfull = nstream * size;
    cal::AlignedVector <uint64_t> data(nfull);
    ...
    cal::rng_multi_dist_uint64(nstream, ndata, mkey1, mkey2, mcounter1,
                                 mcounter2, pdata);

    // Compare this to the result of generating the whole stream serially.

    uint64_t compare[nfull];
    cal::rng_dist_uint64(nfull, key[0], key[1], counter[0], counter[1],
                           compare);
